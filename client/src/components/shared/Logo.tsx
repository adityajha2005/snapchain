...
const Logo = ({ href = '/', iconOnly = false, textClassName, ...props }: LogoProps) => {
  const logoContent = (
    <svg
  ...
  if (href) {
    return (
      <Link href={href} className="flex items-center">
        {logoContent}
        {!iconOnly && (
                  <span className={`ml-2 font-medium text-xl translate-y-0.5 bg-gradient-to-r from-emerald-900 to-black text-transparent bg-clip-text ${textClassName}  `}>SnapChain</span>
        )}
      </Link>
    );
  }
